...
class EdgeSplitter:
    def __init__(self):
        ...
        self.cu_ave = None
        self.L_ave = 0.0
        self.original_vertex_count = 0  # 记录初始顶点数量

    def initialize(self, points: List[Point], faces: List[Triangle]) -> None:
        self.points = points
        self.faces = faces
        self.original_vertex_count = len(points)  # 保存初始顶点数量
    ...
    def _init_histogram_factors(self) -> None:
        ...
        for i in range(len(self.cu_ave)):
            if self.cu_ave[i] < li1:
                self.cu_ave[i] = 1.8 * self.L_ave
            elif self.cu_ave[i] < li2:
                self.cu_ave[i] = 1.4 * self.L_ave
    ...
    def split_edges_subremeshing(self, max_iterations: int = 10) -> Tuple[List[Point], List[Triangle]]:
        ...
        for it in range(max_iterations):
            ...
            for (bs1, bs2, n) in split_list:
                ...
                triangle_vertices = [v for v in bs3_list if v != bs1 and v != bs2 and self._is_valid_index(v, len(points_out))]
...
if __name__ == "__main__":
    ...
    print(len(new_vertices), len(new_faces))
    #new_vertices, new_faces = splitter.split_edges(mode="histogram", max_iter=10)
    writer = PLYWriter()
    writer.write_ply("demo/output/output.ply", new_vertices, new_faces)